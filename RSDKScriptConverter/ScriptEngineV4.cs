...
{
	class ScriptEngineV4 : ScriptEngine
	{
					...
					switch (parseMode)
					{
						case PARSEMODE_SCOPELESS:
							...
							else if (scriptText.StartsWith("publictable"))
							{
								...
								{
									int end = scriptText.IndexOf(']', split + 1);
									line = new ScriptLineTable(true, scriptText.Substring(11, split - 11), scriptText.Substring(split + 1, end - split + 1));
								}
							}
							else if (scriptText.StartsWith("privatetable"))
							{
								...
								{
									int end = scriptText.IndexOf(']', split + 1);
									line = new ScriptLineTable(false, scriptText.Substring(12, split - 12), scriptText.Substring(split + 1, end - split + 1));
								}
							}

							else if (scriptText.Equals("eventObjectUpdate", StringComparison.OrdinalIgnoreCase))
							{
							...
							else if (scriptText.StartsWith("publicfunction"))
							{ // regular public decl
								line = new ScriptLineFunction(true, scriptText.Substring(14));
								parseMode = PARSEMODE_FUNCTION;
							}
							else if (scriptText.StartsWith("privatefunction"))
							{ // regular private decl
								line = new ScriptLineFunction(false, scriptText.Substring(15));
								parseMode = PARSEMODE_FUNCTION;
							}
							break;
				...
				foreach (var line in scriptLines)
				{
					switch (line)
					{
						...
						case ScriptLineObjectUpdate _:
							writer.Write("event ObjectUpdate");
							indent += '\t';
							break;
						...
						case ScriptLineFunction function:
							writer.Write("{0} function {1}", function.Public ? "public" : "private", function.Name);
							indent += '\t';
							break;
